...
MYSQLD="${MYSQLD:-$MYSQL_HOME/bin/mysqld}"
MYSQL="${MYSQL_BIN:-$MYSQL_HOME/bin/mysql}"
MYSQLADMIN="${MYSQLADMIN_BIN:-$MYSQL_HOME/bin/mysqladmin}"
SANDBOX_DIR="${TAWOS_SANDBOX_DIR:-$ROOT_DIR/data/mysql_sandbox}"
DATADIR="$SANDBOX_DIR/data"
SOCKET="$SANDBOX_DIR/mysql.sock"
PID_FILE="$SANDBOX_DIR/mysqld.pid"
LOG_FILE="$SANDBOX_DIR/mysqld.log"
SQL_FILE="${1:-$ROOT_DIR/data/raw/TAWOS.sql}"
OUT_DIR="${TAWOS_OUT_DIR:-$ROOT_DIR/data/processed}"
DB_NAME="${TAWOS_DB_NAME:-tawos_cleaned}"

if [[ ! -x "$MYSQLD" || ! -x "$MYSQL" || ! -x "$MYSQLADMIN" ]]; then
  echo "MySQL binaries not found under $MYSQL_HOME" >&2
  exit 1
fi
...
mkdir -p "$SANDBOX_DIR" "$OUT_DIR"

if [[ ! -d "$DATADIR/mysql" ]]; then
  if [[ -d "$DATADIR" && -n "$(find "$DATADIR" -mindepth 1 -maxdepth 1 2>/dev/null)" ]]; then
    echo "Sandbox data directory is not empty and not initialized: $DATADIR" >&2
    exit 1
  fi

  echo "Initializing isolated MySQL sandbox at $DATADIR..."
  rm -rf "$DATADIR"
  mkdir -p "$DATADIR"
  "$MYSQLD" --no-defaults \
    --initialize-insecure \
...
if [[ -f "$PID_FILE" ]] && kill -0 "$(cat "$PID_FILE")" 2>/dev/null; then
  echo "Sandbox MySQL is already running."
else
  echo "Starting isolated MySQL sandbox..."
  ...
  for _ in {1..60}; do
    if "$MYSQLADMIN" --no-defaults --socket="$SOCKET" -uroot ping >/dev/null 2>&1; then
      break
    fi
    sleep 1
  done
...
if ! "$MYSQLADMIN" --no-defaults --socket="$SOCKET" -uroot ping >/dev/null 2>&1; then
  echo "Sandbox MySQL failed to start. Recent log output:" >&2
...
TABLE_COUNT="$("$MYSQL" --no-defaults --socket="$SOCKET" -uroot --batch --skip-column-names "$DB_NAME" \
  -e "SELECT COUNT(*) FROM information_schema.tables WHERE table_schema = DATABASE();" 2>/dev/null || echo 0)"

if [[ "$TABLE_COUNT" == "0" ]]; then
  echo "Importing $SQL_FILE into sandbox database. This may take several minutes..."
  "$MYSQL" --no-defaults --socket="$SOCKET" -uroot "$DB_NAME" < "$SQL_FILE"
else
  echo "Database already has $TABLE_COUNT tables; skipping import."
fi

echo "Verifying import..."
"$MYSQL" --no-defaults --socket="$SOCKET" -uroot "$DB_NAME" \
  -e "SELECT COUNT(*) AS issue_count FROM Issue; SELECT COUNT(*) AS issue_link_count FROM Issue_Link;"

echo "Exporting task records..."
"$MYSQL" --no-defaults --socket="$SOCKET" -uroot --batch "$DB_NAME" -e "
SELECT
  i.ID AS task_id,
